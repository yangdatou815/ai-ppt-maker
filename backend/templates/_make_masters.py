...
def _patch_theme(prs: Presentation, theme: dict, fonts: dict) -> None:
    ...
    theme_part = prs.slide_masters[0].element.getroottree().getroot()
    ...
    # Write back.
    new_blob = etree.tostring(theme_xml, xml_declaration=True,
                              encoding="UTF-8", standalone=True)
    part._blob = new_blob


def _add_text(slide, left, top, width, height, text, *,
              font_name="Inter", size_pt=18, bold=False, color="0B0B0F",
              align="left"):
...
def _add_sample_slides(prs: Presentation, theme: dict, fonts: dict, name: str) -> None:
    ...
    _add_text(s, Inches(0.6), Inches(2.4), Inches(11), Inches(1.5),
              "Cover Slide Title", font_name=heading_font, size_pt=48,
              bold=True, color=title_color)
    _add_text(s, Inches(0.6), Inches(4.0), Inches(11), Inches(0.6),
              "Subtitle / one-line value proposition",
              font_name=body_font, size_pt=20, color=sub_color)
    _add_text(s, Inches(0.6), Inches(6.7), Inches(11), Inches(0.4),
              f"{name} · master.pptx", font_name=body_font, size_pt=11,
              color=sub_color)

    # 2. TOC -------------------------------------------------------------
    s = prs.slides.add_slide(blank)
    _bg(s, bg)
    _add_text(s, Inches(0.6), Inches(0.5), Inches(11), Inches(0.8),
              "Agenda", font_name=heading_font, size_pt=36, bold=True,
              color=text)
    _accent_rule(s, Inches(0.6), Inches(1.35), Inches(0.8), primary)
    items = ["1. Background", "2. Approach", "3. Results", "4. Next steps"]
    for i, t in enumerate(items):
        _add_text(s, Inches(0.8), Inches(1.9 + i * 0.7), Inches(10), Inches(0.6),
                  t, font_name=body_font, size_pt=22, color=text)

    # 3. Section divider -------------------------------------------------
    s = prs.slides.add_slide(blank)
    _bg(s, primary)
    _add_text(s, Inches(0.6), Inches(2.5), Inches(2), Inches(2),
              "01", font_name=heading_font, size_pt=120, bold=True, color=accent)
    _add_text(s, Inches(2.6), Inches(3.2), Inches(10), Inches(1.2),
              "Section Divider", font_name=heading_font, size_pt=44,
              bold=True, color=bg)
    ...
    _bg(s, bg)
    _add_text(s, Inches(0.6), Inches(0.5), Inches(11), Inches(0.8),
              "Bullets layout", font_name=heading_font, size_pt=32, bold=True,
              color=text)
    ...
    for i, b in enumerate(bullets):
        _add_text(s, Inches(0.8), Inches(1.8 + i * 0.7), Inches(11), Inches(0.6),
                  f"• {b}", font_name=body_font, size_pt=20, color=text)

    # 5. content-image ---------------------------------------------------
    s = prs.slides.add_slide(blank)
    _bg(s, bg)
    _add_text(s, Inches(0.6), Inches(0.5), Inches(11), Inches(0.8),
              "Image layout", font_name=heading_font, size_pt=32, bold=True,
              color=text)
    _accent_rule(s, Inches(0.6), Inches(1.3), Inches(0.6), primary)
    for i, b in enumerate(["• Bullet on the left half",
                           "• Image on the right half",
                           "• Aspect ratio preserved",
                           "• Golden-ratio framing"]):
        _add_text(s, Inches(0.8), Inches(1.9 + i * 0.7), Inches(5.4), Inches(0.6),
                  b, font_name=body_font, size_pt=18, color=text)
    placeholder = s.shapes.add_shape(
        MSO_SHAPE.ROUNDED_RECTANGLE, Inches(7.0), Inches(1.9),
        Inches(5.9), Inches(4.0),
    )
    placeholder.fill.solid(); placeholder.fill.fore_color.rgb = _rgb(text_mute)
    placeholder.line.fill.background()
    _add_text(s, Inches(7.0), Inches(3.7), Inches(5.9), Inches(0.5),
              "[ image placeholder ]", font_name=body_font, size_pt=14,
    ...
    _bg(s, bg)
    _add_text(s, Inches(0.6), Inches(0.5), Inches(11), Inches(0.8),
              "Table layout", font_name=heading_font, size_pt=32, bold=True,
              color=text)
    _accent_rule(s, Inches(0.6), Inches(1.3), Inches(0.6), primary)
    rows, cols = 4, 3
    table_shape = s.shapes.add_table(rows, cols, Inches(0.8), Inches(1.9),
                                     Inches(11.6), Inches(3.5))
    table = table_shape.table
    headers = ["Metric", "Q3", "Q4"]
    data = [["Revenue", "$1.2M", "$1.8M"],
            ["Users",   "12k",   "21k"],
            ["NPS",     "42",    "57"]]
    for c, h in enumerate(headers):
        ...
        run.text = h; run.font.bold = True; run.font.size = Pt(16)
        run.font.color.rgb = _rgb(bg); run.font.name = heading_font
    for r, row in enumerate(data, start=1):
        for c, v in enumerate(row):
            cell = table.cell(r, c)
            cell.text = ""
            run = cell.text_frame.paragraphs[0].add_run()
            run.text = v; run.font.size = Pt(14); run.font.name = body_font
    ...
    _add_text(s, Inches(0.6), Inches(3.2), Inches(11), Inches(1.5),
              "Thank you", font_name=heading_font, size_pt=64, bold=True,
              color=title_color)
    _add_text(s, Inches(0.6), Inches(4.6), Inches(11), Inches(0.6),
              "Questions & discussion",
              font_name=body_font, size_pt=22, color=text_mute)
...
def main() -> int:
    targets = [d for d in sorted(HERE.iterdir())
               if d.is_dir() and (d / "layout-mapping.yaml").is_file()]
    if not targets:
    ...
    for d in targets:
        ...
        print(f"[OK] {out.relative_to(HERE.parent.parent)}  ({size_kb} KB, "
